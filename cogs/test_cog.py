...
class Menus(commands.Cog):
	...
	async def find_me_a_resource(self, ctx):
		...
		async def button_callback(interaction):
			...
			def get_name_only(dict):
				return dict["name"]
			name_of_resources = list(map(get_name_only, resources))
			print(name_of_resources)
			if resources != []:
				embed = discord.Embed(colour = discord.Colour.purple())
				embed.set_author(name=f"Let's see, {response} {resource_type}....")
				embed.add_field(name="Here's what I found", value=str(name_of_resources[0:10]), inline=False)
				await ctx.respond(embed=embed)
			else:
				embed = discord.Embed(colour = discord.Colour.red())
				embed.set_author(name=f"I can't seem to find {response} {resource_type}.")
				embed.add_field(name="You might want to check your spelling.", value="Try /find_me_a_resource", inline=False)
				await ctx.respond(embed=embed)
